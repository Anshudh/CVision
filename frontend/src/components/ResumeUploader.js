...
const ResumeUploader = () => {
  ...
  const slowAnalysisTimerRef = useRef(null);

  useEffect(() => {
    if (loading) {
      slowAnalysisTimerRef.current = setTimeout(() => {
        setSlowAnalysis(true);
      }, 12000);
    } else {
      setSlowAnalysis(false);
      if (slowAnalysisTimerRef.current) {
        clearTimeout(slowAnalysisTimerRef.current);
        slowAnalysisTimerRef.current = null;
      }
    }
    ...
    <div className="resume-uploader">
      {!result ? (
        <div className="upload-section">
          ...
          {loading && slowAnalysis && (
            <div className="slow-analysis-message">
              This is taking longer than usual. The analysis is still running and should finish shortly.
            </div>
          )}
